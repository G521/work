...
public class spider_traslate {
    private static HashMap<String,String>langMap=dissmsg.langMap;
    ...
    public static String result(String info) {
        //接口地址
        if (info.startsWith("#")){
            return changeLanguage(info.substring(1));
        }
...
//        params.put("sign","0d3389d9dea66ae1");
    ...
    public static String changeLanguage(String str){
        if (langMap.containsKey(str)){
            to=str;
           return "已经设置转换语言到"+str;

        }else {
           return "抱歉，请检查输入语言名称是否有误";
        }
    }
    public static String showLanguage(){
    ...
    public static void main(String[] args) {
        String result= spider_traslate.result("你好");
