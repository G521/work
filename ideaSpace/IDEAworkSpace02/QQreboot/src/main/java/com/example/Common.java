 ...
 * 具体功能可以查看文档
 */
public class Common extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
    private dissmsg dissmsg;
    File file=new File("F:\\maomao");
    /**
    ...
    public static void main(String[] args) {
        ...
        CQ = new CQDebug("D:\\CQA-tuling\\酷Q Air\\data\\app\\com.sobte.cqp.jcq\\app\\com.example.common\\","data");//new CQDebug("应用目录","应用名称") 可以用此构造器初始化应用的目录
...
////         开始模拟群聊消息
////        demo.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "#语言列表", 0);
//////        demo.groupMsg(0, 10008, 3456789012L, 11111111114L, "", "&今日单词", 0);
////
////        demo.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "&#日语", 0);
////         //依次类推，可以根据实际情况修改参数，和方法测试效果
////
////        demo.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "#你好", 0);
////       //  以下是收尾触发函数
...
//        demo.exit();// 最后程序运行结束，调用exit方法
    ...
    public String appInfo() {
        ...
        String AppID = "com.example.common";// 记住编译后的文件和json也要使用appid做文件名
        /**
    ...
    public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
        // 这里处理消息
        if (msg.startsWith("#")){
            CQ.sendPrivateMsg(fromQQ,translation.translate(msg.substring(1)));
        }
        return MSG_IGNORE;
    }
    /**
...
//        CQ.sendGroupMsg(fromGroup, CC.at(fromQQ) + "你发送了这样的消息：" + msg);
          ...
          dissmsg.receive(str,fromGroup,fromQQ);
      }
      if (msg.startsWith("#")){
          dissmsg.fanyi(msg.substring(1),fromGroup,fromQQ);
      }
        return MSG_IGNORE;
    }
    /*
    ...
    public int groupAdmin(int subtype, int sendTime, long fromGroup, long beingOperateQQ) {
        // 这里处理消息
        CQ.sendGroupMsg(fromGroup,dissmsg.getName(beingOperateQQ)+"被设置管理了，不行，我也要~");
        return MSG_IGNORE;
    }
    /**
    ...
    public int groupMemberDecrease(int subtype, int sendTime, long fromGroup, long fromQQ, long beingOperateQQ) {
        // 这里处理消息
        CQ.sendGroupMsg(fromGroup,"欧！一位群成员"+(subtype==1?"走了。。。":"被踢了~！活该~~~"));
        return MSG_IGNORE;
    }
    /**
    ...
    public int groupMemberIncrease(int subtype, int sendTime, long fromGroup, long fromQQ, long beingOperateQQ) {
        // 这里处理消息
        if (fromQQ==903597424)return MSG_IGNORE;
        CQ.sendGroupMsg(fromGroup,"欢迎"+beingOperateQQ+"入群~");
        return MSG_IGNORE;
    }
    ...
    public int friendAdd(int subtype, int sendTime, long fromQQ) {
        // 这里处理消息
        CQ.sendPrivateMsg(fromQQ,"私聊系统还没主人还没设置好呢");
        return MSG_IGNORE;
    }
    /**
     * 请求-好友添加 (Type=301)<br>
...
//		}
		if(subtype == 2){
		    ...
		    CQ.sendGroupMsg(fromGroup,"大家好！ 我叫小高~，有事可以用%呼唤我哦~\n 可以先设置名字哦：\n例如：\n%名字张张\n也可以查名字哦~\n例如：\n%查名字");
		    CQ.sendGroupMsg(fromGroup,"现在只有这一种功能呢~");
		}
